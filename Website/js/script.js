...
function canEdit() {
    return hasPermission('file.edit') || hasPermission('file.delete') || isAdmin();
}
    ...
    let fileHtml = `
        <div class="file-item" data-file-id="${file.file_id}">
            ...
            <div style="display: flex; align-items: center; gap: 12px;">
                ${hasPermission('file.download') && !isExpiredFile ? `
...
function displayFiles(files) {
    ...
    if (!activeFilesContainer || !inactiveFilesContainer) {
        console.error('File containers not found');
        return;
    }
    ...
    if (activeCountEl) activeCountEl.textContent = activeFiles.length;
    if (expiredCountEl) expiredCountEl.textContent = expiredFiles.length;
    if (revokedCountEl) revokedCountEl.textContent = '0';
...
async function loadDashboardMetrics() {
    try {
        ...
        // Load files and activity logs in parallel
        const [filesResponse, activityResponse] = await Promise.all([
            ...
            fetch('api.php', {
                ...
                body: (() => {
                    ...
                    formData.append('token', token);
                    formData.append('risk_level', 'all');
                    formData.append('action_type', 'all');
        ...
        let activityData = { success: false, stats: null };
        
        // Try to get activity data, but don't fail if it errors
        try {
            activityData = await activityResponse.json();
        } catch (activityError) {
            console.warn('Could not load activity stats:', activityError);
        }

        // Always update metrics, even if one request fails
        const files = (filesData.success && filesData.files) ? filesData.files : [];
        const activityStats = (activityData.success && activityData.stats) ? activityData.stats : null;
        
        console.log('Dashboard metrics loaded:', {
            filesCount: files.length,
            activityStats: activityStats,
            activitySuccess: activityData.success,
            filesSuccess: filesData.success
        });
        
        updateDashboardMetrics(files, activityStats);
    } catch (error) {
        console.error('Error loading dashboard metrics:', error);
    }
}

// Update dashboard metrics (make it globally accessible)
window.updateDashboardMetrics = function(files, activityStats = null) {
    const now = new Date();
    const activeFiles = [];
    const expiredFiles = [];
    ...
    // Get activity stats if provided, otherwise use defaults
    const highRiskCount = activityStats ? (Number(activityStats.high_risk) || 0) : 0;
    const securityEventsCount = activityStats ? (Number(activityStats.total_events) || 0) : 0;
    ...
    console.log('Updating dashboard metrics:', {
        ...
        activityStatsProvided: !!activityStats,
        activityStatsValue: activityStats
    });
    ...
    if (encryptedCountEl) {
        encryptedCountEl.textContent = encryptedFiles;
        console.log('Set encrypted files count to:', encryptedFiles);
    }
    if (encryptionRateEl) encryptionRateEl.textContent = `encryption rate ${encryptionRate}%`;
    if (activeFilesCountEl) {
        activeFilesCountEl.textContent = activeFiles.length;
        console.log('Set active files count to:', activeFiles.length);
    }
    if (totalFilesTextEl) totalFilesTextEl.textContent = `of ${totalFiles} total`;
    if (highRiskCountEl) {
        highRiskCountEl.textContent = highRiskCount;
        console.log('Set high risk count to:', highRiskCount, 'from stats:', activityStats);
    }
    if (securityEventsCountEl) {
        securityEventsCountEl.textContent = securityEventsCount;
        console.log('Set security events count to:', securityEventsCount, 'from stats:', activityStats);
    }
    ...
    if (alertsContainer) {
        ...
        if (alertsHtml === '') {
            alertsHtml = '<div class="empty-state"><div style="font-size: 36px; margin-bottom: 12px; opacity: 0.5;">✅</div><div style="font-weight: 600; margin-bottom: 4px; color: #065f46;">All Clear!</div><div style="color: #6b7280; font-size: 13px;">No security alerts at this time</div></div>';
        }
...
if (window.location.pathname.includes('dashboard.html') || window.location.href.includes('dashboard.html')) {
    // Dashboard initialization is handled in dashboard.html's inline script
    // This ensures proper order of execution
}

// Load files when files.html page loads
if (window.location.pathname.includes('files.html') || window.location.href.includes('files.html')) {
    document.addEventListener('DOMContentLoaded', function() {
        fetchFiles();
    });
}
